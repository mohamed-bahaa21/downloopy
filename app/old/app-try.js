...
function downloadTenByTen(START_NUM, FINISH_NUM, FOLDER_NAME, FILE_NAME, total_dir, URI_START, URI_END, FILE_TYPE, CHNKS) {
    if(CHNKS > 0){
        for (let index = 0; index <= 10; index++) {
            const element = index + START_NUM;
            console.log(element);
        ...
        CHNKS = CHNKS - 1;
        downloadTenByTen(START_NUM, FINISH_NUM, FOLDER_NAME, FILE_NAME, total_dir, URI_START, URI_END, FILE_TYPE, CHNKS);
    } else {
        console.log('There is no other chunks...')
        return;
    }
}
...
downloadAllPost = (req, res, next) => {
    ...
    // use this if numbers system is 01 
    // don't use it if it's normal system
